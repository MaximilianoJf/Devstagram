...
@section('contenido')
    ...
    <div class="container mx-auto md:flex p-5">
        <div class="md:w-1/2 p-1">
            ...
            <div id="likeSection" class="p-3 flex items-center gap-4">
             
                <livewire:like-post :post="$post"/>     
                {{-- <livewire:like-post :post="$post" /> --}}
                    {{-- checkLike debe devolver un true --}}{{--El delete de method es Metodo spofing --}}
                ...
                <p class="font-bold">{{$post->likes->count()}} 
                    <span class="font-normal">Likesssss</span></p> --}}
            </div>
            <div>
                <p class="font-bold">{{$post->user->username}}</p>     
                <p class="text-sm text-gray-500">
                    {{-- appi fechas difforhumans --}}
            ...
            @auth
                @if($post->user_id === auth()->user()->id)
                    <form method="POST" action="{{route('posts.destroy', $post)}}">
                        @method('DELETE')   {{-- Metodo Spoofing: el navegador nativamente solo soporta get y
                             post y esto permite agregar delete, patch... --}}
                        @csrf
                        <input
                            type="submit"
            ...
            <div class="bg-white p-5 mb-5 rounded-lg">
                @auth
                    ...
                    <form action="{{route('Comentarios.store',['post' => $post, 'user' => $user])}}" method="POST">
                        ...
                        <input type="submit" value="Comentario" class="bg-sky-600
